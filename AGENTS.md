...
This is a **Dev Container Features** repository implementing the [Dev Container specification](https://containers.dev/). It creates custom dev container features published to GitHub Container Registry (GHCR).
...
devcontainer features test --skip-scenarios -f <feature-name> -i mcr.microsoft.com/devcontainers/base:ubuntu .
...
1. File issues or TODOs for any known follow-up work when appropriate.
2. Run relevant quality gates if code changed (tests, linters, builds, validation).
3. Commit completed work with a clear message.
4. Push changes to the remote branch.
5. Verify `git status` is clean and the branch is up to date with its upstream.
