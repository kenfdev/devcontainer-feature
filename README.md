...
- [`tailscale-dev`](./src/tailscale-dev) - installs Tailscale and configures an entrypoint for Tailscale SSH access to a dev container.
...
Installs Tailscale and uses `/usr/local/bin/tailscale-entrypoint.sh` to start `tailscaled`, run `tailscale up`, and then execute the container command.
...
For Docker and Docker Compose usage, see [`src/tailscale-dev/README.md`](./src/tailscale-dev/README.md).
...
# Run tests for the tailscale-dev feature
devcontainer features test -f tailscale-dev --skip-autogenerated --skip-duplicated .
