...
# Test takt-only installation - only takt should be installed
echo "Testing takt-only installation..."

# Test takt (should be installed)
if command -v takt &>/dev/null; then
    echo "PASS: takt is installed"
else
    echo "FAIL: takt is not installed"
    exit 1
fi
